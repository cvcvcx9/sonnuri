          ...
          <div className="p-6">
              ...
              <div className="bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 p-4">
                  <input
                    ...
                    onChange={(e) => handleSingleCheck('basic', e.target.checked)}
                    className="w-5 h-5"
                  />
                  ...
                  >
                    {expanded.basic ? <ChevronUp className="h-5 w-5"/> : <ChevronDown className="h-5 w-5" />}
                  </button>
                </div>
                {expanded.basic && (
                  <div className="p-4 text-sm bg-white rounded-b-lg border-t border-gray-300 space-y-4">
                    ...
                    <p>① 거래처는 실명으로 거래하여야 한다.<br />
                    ② 은행은 거래처의 실명확인을 위하여 주민등록증ㆍ사업자등록증 등 실명확인증표 또는 그 밖에 필요한 서류의 제시나 제출을 요구할 수 있고, 거래처는 이에 따라야 한다.</p>
                    
                    <h3 className="font-bold mb-2">제3조(거래장소)</h3>
                    <p>거래처는 예금계좌를 개설한 영업점(이하 "개설점"이라 한다)에서 모든 예금거래를 한다. 다만, 은행이 정하는 바에 따라 다른 영업점이나 다른 금융기관, 또는 전산통신기기를 통하여 거래할 수 있다.</p>
                    
                    <h3 className="font-bold mb-2">제4조(거래방법)</h3>
                    <p>거래처는 은행에서 내준 통장(증서· 전자통장을 포함한다) 또는 수표· 어음용지로 거래하여야 한다. 그러나 입금할 때, 자동이체약정ㆍ전산통신기기이용약정에 따라 거래하는 경우 통장 없이도 거래할 수 있다.</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 p-4">
                  <input
                    ...
                    onChange={(e) => handleSingleCheck('savings', e.target.checked)}
                    className="w-5 h-5"
                  />
                  ...
                  <button
                    ...
                    className="ml-auto text-gray-500 hover:text-gray-700"
                  >
                    {expanded.savings ? <ChevronUp className="h-5 w-5"/> : <ChevronDown className="h-5 w-5" />}
                  </button>
                </div>
                {expanded.savings && (
                  <div className="p-4 text-sm bg-white rounded-b-lg border-t border-gray-300 space-y-4">
                    <h3 className="font-bold mb-2">제1조(적용범위)</h3>
                    <p>① 적립식예금(이하 "이 예금"이라 한다)이란 기간을 정하고 그 기간 중에 미리 정한 금액이나 불특정금액을 입금하는 예금을 말한다.<br />
                    ② 이 약관에 정하지 않은 사항은 예금거래기본약관의 규정을 적용한다.</p>
                    <h3 className="font-bold mb-2">제2조(지급시기)</h3>
                    <p>이 예금은 약정한 만기일 이후 거래처가 청구할 때 지급한다. 다만, 거래처가 부득이한 사정으로 청구할 때는 만기전이라도 지급할 수 있다.</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 p-4">
                  <input
                    ...
                    onChange={(e) => handleSingleCheck('special', e.target.checked)}
                    className="w-5 h-5"
                  />
                  ...
                  <button
                    ...
                    className="ml-auto text-gray-500 hover:text-gray-700"
                  >
                    {expanded.special ? <ChevronUp className="h-5 w-5"/> : <ChevronDown className="h-5 w-5" />}
                  </button>
                </div>
                {expanded.special && (
                  <div className="p-4 text-sm bg-white rounded-b-lg border-t border-gray-300 space-y-4">
                    <h3 className="font-bold mb-2">제1조(적용범위)</h3>
                    <p>이 특약은 고객과 은행 사이의 급여하나 월복리 적금 거래에 적용됩니다.</p>
              ...
              <div className="bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 p-4">
                  <input
                    ...
                    onChange={(e) => handleSingleCheck('product', e.target.checked)}
                    className="w-5 h-5"
                  />
                  ...
                  <button
                    ...
                    className="ml-auto text-gray-500 hover:text-gray-700"
                  >
                    {expanded.product ? <ChevronUp className="h-5 w-5"/> : <ChevronDown className="h-5 w-5" />}
                  </button>
                </div>
                {expanded.product && (
                  <div className="p-4 text-sm bg-white rounded-b-lg border-t border-gray-300 space-y-4">
                    ...
                    <p>• 상품명: 알.쏠 적금<br />
                    • 상품특징: 가입 기간을 선택하여 월 납입 한도 내 신규한 원금이 자유롭고, 가계 실적에 따라 우대금리를 제공하는 적금</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 p-4">
                  <input
                    ...
                    onChange={(e) => handleSingleCheck('rights', e.target.checked)}
                    className="w-5 h-5"
                  />
                  <span className="text-blue-600">[필수] 금융소비자의 권리 안내</span>
                  <button
                    onClick={() => toggleExpand('rights')}
                    className="ml-auto text-gray-500 hover:text-gray-700"
                  >
                    {expanded.rights ? <ChevronUp className="h-5 w-5"/> : <ChevronDown className="h-5 w-5" />}
                  </button>
                </div>
                {expanded.rights && (
                  <div className="p-4 text-sm bg-white rounded-b-lg border-t border-gray-300 space-y-4">
                    <h3 className="font-bold mb-2">1. 청약철회권</h3>
                    <p>일반금융소비자는 청약철회가 가능한 금융상품에 대하여 청약을 철회할 수 있습니다.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-8 p-4 bg-gray-50 rounded-lg text-sm">
              <p className="text-gray-600">
                본인은 위 예금상품의 약관 및 상품설명서를 제공받고 중요한 사항을 충분히 이해하였음을 확인합니다.
              </p>
            </div>
